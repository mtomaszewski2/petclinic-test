...
public class VisitRestController {
    ...
    public ResponseEntity<VisitDto> addVisit(@RequestBody @Valid VisitDto visitDto, BindingResult bindingResult, UriComponentsBuilder ucBuilder) {
        ...
        if(visitDto.getPetId() == null) {
            headers.add("errors", new Error("Missing pet id").toJson());
            return new ResponseEntity<>(headers, HttpStatus.BAD_REQUEST);
        }

        Pet pet = clinicService.findPetById(visitDto.getPetId());
        if(isNull(pet)) {
            headers.add("errors", new Error("Pet does not exist in the system.").toJson());
